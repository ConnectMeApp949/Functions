...
def getMessages_fn(request):
    try:
        ...
        limit = min(int(req_json.get('limit', 20)), 100)  # Cap limit to prevent abuse
        ...
        # Ownership check: verify user is a participant in this thread
        user_id = req_json.get('userId')
        thread_doc = fdb.collection(thread_collection).document(thread_id).get()
        if thread_doc.exists:
            thread_data = thread_doc.to_dict()
            if user_id not in thread_data.get('userIds', []):
