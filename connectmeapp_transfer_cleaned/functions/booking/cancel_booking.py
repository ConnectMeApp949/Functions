...
def cancelBooking_fn(request):
    ...
    try:
        ...
        # Ownership check: only booking participants can cancel
        if user_id != client_user_id and user_id != vendor_user_id:
            return jsonify({'success': False, 'reason': 'Unauthorized'}), 403

        # Derive userType from booking data instead of trusting client
        cancel_request_user = "client" if user_id == client_user_id else "vendor"
