...
def confirmBookingAndPay_fn(request):
    ...
    try:                
        ...
        # Ownership check: only the vendor can confirm a booking
        user_id = pdata.get("userId")
        if user_id != vendor_user_id:
            return jsonify({'success': False, 'reason': 'Unauthorized: only vendor can confirm'}), 403
